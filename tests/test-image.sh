...
run_static_tests() {
    ...
    check "source label is set" \
        '$RUNTIME inspect --format="{{index .Config.Labels \"org.opencontainers.image.source\"}}" "$IMAGE" | grep -q "crunchtools/zabbix-agent"'
}
...
run_runtime_tests() {
    ...
    local CONTAINER_NAME="zabbix-agent-test-$$"
